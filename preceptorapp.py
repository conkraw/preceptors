...
with col3:
    ...
    if redcapmetrics is not None:
        if st.button ("Next - Process REDCAP Metrics", key= "next_redcap"): 
            try:
                ...
                if redcapmetrics.name.endswith("csv"):
                    ...
                    st.error("Unsupported file type for REDCAP Metrics")
                    dfe = None
...
with col1:
    ...
    if evaluation_due_dates_file is not None:
        if st.button("Next - Process Evaluation Due Dates", key="next_eval"):
            try:
                # Determine the file type and load accordingly
...
if analysis_report_file is not None:
    if st.button("Next - Process Analysis Report", key="next_analysis"):
        try:
            # Determine the file type and load accordingly
